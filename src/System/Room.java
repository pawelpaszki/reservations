	...
	public boolean isAvailable(Date startDate, Date endDate) {
		...
		for (int month = startDate.getMonth(); month <= endDate.getMonth();) {
			for (int day = startDate.getDay();; day++) {
				if(month == endDate.getMonth() && day > endDate.getDay()) {
					return true;
				}
				if((monthWith31.contains(Integer.valueOf(month)) && day >31) || (monthWith30.contains(Integer.valueOf(month)) && day >30) || (day >28 && month == 2)) {
					month++;
					day = 1;
				} 
